...
// Enqueues data to be sent via LoRa Peer-to-Peer
static inline void enqueue_lora_p2p_data(CommunicationUnit *data_unit);

static ChannelAPI lora_p2p_api;
static LoraCommonAPI lora_p2p_common_api = {
	.channel_type = LORA_P2P,
	.pulga_buffer = &lora_p2p_buffer,
...
static int lora_p2p_init_channel(void)
{
	...
	lora_p2p_common_api.send_thread_id = k_thread_create(&lora_p2p_send_thread_data, lora_p2p_send_thread_stack_area,
	...
	error = k_thread_name_set(lora_p2p_common_api.send_thread_id, "send_lora_p2p");
	lora_p2p_send_thread_id = lora_p2p_common_api.send_thread_id;
	if (error)
...
static inline void enqueue_lora_p2p_data(CommunicationUnit *data_unit)
{
	...
	do
	{
		...
		error = encode_and_enqueue(&lora_p2p_common_api, *data_unit, MINIMALIST);
	} while (error);
...
ChannelAPI *register_lora_p2p_callbacks()
{
	...
	lora_p2p_api.enqueue_data = enqueue_lora_p2p_data;
	return &lora_p2p_api;
}